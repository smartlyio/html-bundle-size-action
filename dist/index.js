...
exports.assetSize = exports.absolutePath = exports.linkedAssets = void 0;
const fs = __importStar(__nccwpck_require__(7147));
const http = __importStar(__nccwpck_require__(3685));
const https = __importStar(__nccwpck_require__(5687));
const cheerio_1 = __importDefault(__nccwpck_require__(4612));
const url_1 = __nccwpck_require__(7310);
const scriptAssets = ($) => $('script[src]')
    .toArray()
    .map(el => ({
    type: 'script',
    href: el.attribs.src,
...
const styleAssets = ($) => $('link[rel=stylesheet]')
    .toArray()
    .map(el => ({
    type: 'style',
...
const linkedAssets = (html) => {
    const $ = cheerio_1.default.load(html);
    return [...scriptAssets($), ...styleAssets($)];
};
exports.linkedAssets = linkedAssets;
const absolutePath = (path, base) => {
    if (base) {
...
const assetSize = (asset) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        request(asset.href, res => {
            if (res.statusCode !== 200) {
                ...
                resolve(Object.assign(Object.assign({}, asset), { size: Number(res.headers['content-length']) }));
            }
        })
            .on('error', err => {
            reject(err);
...
const bundleSize = (options) => __awaiter(void 0, void 0, void 0, function* () {
    const html = yield fs.promises.readFile(options.path, 'utf8');
    const assets = (0, exports.linkedAssets)(html)
        .filter(({ async }) => !async)
        .map(asset => (Object.assign(Object.assign({}, asset), { href: (0, exports.absolutePath)(asset.href, options.base) })));
    const sizes = yield Promise.all(assets.map(exports.assetSize));
    return sizes.reduce((acc, { size }) => acc + size, 0);
});
exports["default"] = bundleSize;
...
const bundle_size_1 = __importDefault(__nccwpck_require__(1367));
function run() {
